...
export const renderSkeletons = (num: number): JSX.Element[] => {
    const skeletons = [];
    for (let i = 0; i < num; i++) {
        skeletons.push(<YoutubeThumbnailSkeleton />);
    }
    return skeletons;
}
...
export const YoutubeThumbnail: React.FC<YoutubeThumbnailProps> = ({ src, alt, title, videoId }) => {
    ...
    return (
        <div className="youtube-thumbnail" onClick={e => handleThumbnailClick()}>
            ...
            <a href={`https://www.youtube.com/watch?v=${videoId}`} onClick={(e) => e.stopPropagation()} target="_blank" rel="noopener noreferrer" ><div className="title">{title}</div></a>
        </div>
    );
}